...
subprocess.run(shlex.split(f'python main.py --lr {0.1} --weight_decay {0}   --logdir {"./log_tmp/test_save/debug_base.log"}  --use_list {"leader_one"} --test --process_data --use_base'))
...
for lr in lr_li:
    for weight_decay in weight_decay_li:
        try:
            subprocess.run(shlex.split(f'python main.py --lr {lr} --weight_decay {weight_decay}   --logdir {"./log_tmp/test_save/base.log"}  --use_list {"leader_one"} --test --use_base'),check=True)
        except subprocess.CalledProcessError:
            file_logger.error('something wrong in run.py')