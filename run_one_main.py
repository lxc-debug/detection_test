...
# subprocess.run(shlex.split(f'python main.py --lr {0.001} --weight_decay {0.0001}   --logdir {"./log_tmp/main/debug_base.log"}  --use_list {"leader_one"} --main --process_data --use_base --use_q_node'))
...
for weight_decay in weight_decay_li:
    for lr in lr_li:
        try:
            subprocess.run(shlex.split(f'python main.py --lr {lr} --weight_decay {weight_decay}   --logdir {"./log_tmp/main/base.log"}  --use_list {"leader_one"} --main --use_base --use_q_node'),check=True)
        except subprocess.CalledProcessError:
            file_logger.error('something wrong in run.py')