...
with open('log_tmp/save/all_archi_base.log',mode='r') as fp:
    all_archi_base=fp.read()

with open('log_tmp/save/all_archi_bin.log',mode='r') as fp:
    all_archi_bin=fp.read()

with open('log_tmp/save/all_noarchi_base.log',mode='r') as fp:
    all_noarchi_base=fp.read()

with open('log_tmp/save/all_noarchi_bin.log',mode='r') as fp:
    all_noarchi_bin=fp.read()

with open('log_tmp/save/all_archi_base_useq.log',mode='r') as fp:
    all_archi_base_useq=fp.read()

with open('log_tmp/save/all_archi_bin_useq.log',mode='r') as fp:
...
# res=re.findall(r'train_acc:\s+(\d+\.\d+)[^\n]*eval_acc:\s+(\d+\.\d+)[^\n]*\n[^\n]*test dataset acc:\s+(\d+\.\d+)[^\n]*auc_roc_score:(\d+\.\d+)',all_archi_base)
# train_acc,eval_acc,test_acc,auc_roc_score=max(res,key=lambda x:float(x[1]))
# print(f'{"all_archi_base":20} train_acc:{train_acc}|eval_acc:{eval_acc}|test_acc:{test_acc}|auc_roc:{auc_roc_score}')

# res=re.findall(r'train_acc:\s+(\d+\.\d+)[^\n]*eval_acc:\s+(\d+\.\d+)[^\n]*\n[^\n]*test dataset acc:\s+(\d+\.\d+)[^\n]*auc_roc_score:(\d+\.\d+)',all_archi_bin)
# train_acc,eval_acc,test_acc,auc_roc_score=max(res,key=lambda x:float(x[1]))
# print(f'{"all_archi_bin":20} train_acc:{train_acc}|eval_acc:{eval_acc}|test_acc:{test_acc}|auc_roc:{auc_roc_score}')

# res=re.findall(r'train_acc:\s+(\d+\.\d+)[^\n]*eval_acc:\s+(\d+\.\d+)[^\n]*\n[^\n]*test dataset acc:\s+(\d+\.\d+)[^\n]*auc_roc_score:(\d+\.\d+)',all_noarchi_base)
# train_acc,eval_acc,test_acc,auc_roc_score=max(res,key=lambda x:float(x[1]))
# print(f'{"all_noarchi_base":20} train_acc:{train_acc}|eval_acc:{eval_acc}|test_acc:{test_acc}|auc_roc:{auc_roc_score}')

# res=re.findall(r'train_acc:\s+(\d+\.\d+)[^\n]*eval_acc:\s+(\d+\.\d+)[^\n]*\n[^\n]*test dataset acc:\s+(\d+\.\d+)[^\n]*auc_roc_score:(\d+\.\d+)',all_noarchi_bin)
# train_acc,eval_acc,test_acc,auc_roc_score=max(res,key=lambda x:float(x[1]))
# print(f'{"all_noarchi_bin":20} train_acc:{train_acc}|eval_acc:{eval_acc}|test_acc:{test_acc}|auc_roc:{auc_roc_score}')